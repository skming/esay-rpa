...
function initUpdater() {
  ...
  autoUpdater.on('checking-for-update', () => {
    sendToRenderer(IPC_CHANNELS.update.status, { status: 'checking' });
  });

  autoUpdater.on('update-available', (info) => {
    sendToRenderer(IPC_CHANNELS.update.status, {
      status: 'available',
  ...
  return {
    checkForUpdates() {
      if (!app.isPackaged) {
        sendToRenderer(IPC_CHANNELS.update.status, { status: 'error', error: 'update-dev-mode' });
        return;
      }
      autoUpdater.checkForUpdates().catch((err) => {
        sendToRenderer(IPC_CHANNELS.update.status, { status: 'error', error: err.message });
      });
    },
    downloadUpdate() {
