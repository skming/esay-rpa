...
async function generateScraplingScript(payload = {}, backendClient = new BackendClient()) {
  ...
  const targetUrl = sanitizePythonString(payload.targetUrl ?? '');
  const selector = sanitizePythonString(payload.selector ?? '');
  ...
  return {
    ...
    content: [
      ...
      '',
      'def run() -> None:',
      `    """运行从 Easy RPA 生成的 ${flowName} 采集脚本。"""`,
      `    page = Fetcher.get("${targetUrl}")`,
      `    element = page.css("${selector}")`,
      '    if element is None:',
      `        raise RuntimeError("未找到目标元素: ${selector}")`,
      '    print(element.text())',
      '',
      '',
