...
export function createNodeConfigDraft(data: RpaNodeData): RpaNodeConfigDraft {
  ...
  return {
    ...
    indexVariable: data.action?.indexVariable ?? 'loop_index',
    maxIterations: data.action?.maxIterations ?? 1000,
...
function getDefaultPath(actionType: string, kind: RpaNodeData['kind'], existingCode?: string): string {
  // Don't show a default path placeholder when the node uses inline code
...
export function applyNodeConfigDraft(data: RpaNodeData, draft: RpaNodeConfigDraft): RpaNodeData {
  ...
  return {
    ...
    action: {
      ...
      indexVariable: data.action?.type === 'control.foreach' ? (draft.indexVariable === '' ? undefined : draft.indexVariable) : data.action?.indexVariable,
      maxIterations: data.action?.type === 'control.foreach' || data.action?.type === 'browser.clickLoadMore' || data.action?.type === 'browser.paginateNext' || data.action?.type === 'browser.dismiss' ? draft.maxIterations : data.action?.maxIterations,
      ...
      targetSelector: data.action?.type === 'ui.drag' || data.action?.type === 'browser.drag' || data.action?.type === 'browser.clickLoadMore' || data.action?.type === 'browser.paginateNext' || data.action?.type === 'browser.dismiss' ? (draft.targetSelector === '' ? undefined : draft.targetSelector) : data.action?.targetSelector,
      ...
      countVariable: data.action?.type?.startsWith('excel.') || data.action?.type?.startsWith('file.') ? (draft.statusVariable === '' ? undefined : draft.statusVariable) : data.action?.countVariable,
...
function shouldUseTargetUrl(actionType: string | undefined): boolean {
  return actionType === 'browser.fetch' || actionType === 'browser.open' || actionType === 'browser.tab.open';
}
