...
export function buildNodeIoSummary(node: Node<RpaNodeData>): NodeIoSummary {
  ...
  if (actionType === 'browser.ensureLogin') {
    addOutput(action.firstValueVariable, 'String', '登录态（logged_in / login_required）');
  }
  ...
  if (actionType === 'browser.fill' || actionType === 'ui.fill' || actionType === 'browser.press' || actionType === 'browser.click' || actionType === 'ui.click' || actionType === 'browser.wait' || actionType === 'browser.waitFor' || actionType === 'ui.wait' || actionType === 'browser.screenshot' || actionType === 'ui.screenshot' || actionType === 'browser.tab.close' || actionType === 'browser.tab.switch' || actionType === 'browser.scroll' || actionType === 'browser.select' || actionType === 'ui.select' || actionType === 'browser.check' || actionType === 'ui.check' || actionType === 'browser.drag' || actionType === 'ui.drag') {
  ...
  if (actionType === 'control.repeat_until') {
    addOutput(action.indexVariable, 'Integer', '已执行轮数');
  }
  ...
  if (actionType.startsWith('variable.')) {
    ...
    if (actionType === 'variable.set' && typeof action.variableName === 'string') {
