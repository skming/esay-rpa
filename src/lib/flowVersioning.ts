...
/** Bumps the patch version past the highest existing version among flows sharing this name. */
...
/** Fingerprint for change detection; `exportedAt` is excluded since it changes on every export regardless of logic changes. */
...
/** Sorts object keys so insertion order doesn't affect the result — safe as a change fingerprint. */
