...
/** Parses the JSON string set on a drag-start event by the component palette. */
...
/** Creates a new canvas node snapped to a 10 px grid at the given drop position. */
...
/**
 * Returns true when a new edge between `source` and `target` is allowed.
 * Prevents self-loops, edges into 'start', edges from 'end', and duplicates.
 */
...
/**
 * Splices a new node between `sourceNodeId` and its current successor.
 * The existing outgoing edge is re-routed through the new node so the flow
 * remains connected.
 */
...
/**
 * Removes a node and attempts to preserve flow connectivity by wiring its
 * predecessor directly to its successor. The 'start' and 'end' sentinel nodes
