...
export function createBrowserBridge({ backendClient = new BackendClient() }: BrowserBridgeOptions = {}): RpaBridge {
  ...
  const pollBackendTask = async (runId: string): Promise<void> => {
    ...
    try {
      ...
      // Poll-based fallback: surface the input dialog when the backend is waiting for user input.
      // This catches cases where the WebSocket log with level='input' was missed or the socket
      // was not yet connected when the backend appended the log.
    ...
    try {
      ...
      socket.addEventListener('message', (event) => {
        ...
        try {
          ...
          // backend 在 task not found 时发送 {"type":"error","message":"..."} — 无 id 字段，不是日志条目
          if (typeof parsed.id !== 'string') {
            return;
          }
          emitBackendLog(run, emitRunEvent, parsed as unknown as BackendTaskLogEntry);
        } catch {
          // JSON 解析失败：静默丢弃，HTTP 轮询会补全日志
        }
      });
      ...
      // WebSocket 只是实时日志加速通道，失败时继续依赖 HTTP 轮询。
    }
  };
  ...
  return {
    ...
    getAiConfig: async (): Promise<BridgeResult<AiConfig>> => {
      try {
        const res = await fetch(`${backendClient.baseUrl}/api/ai/config`);
        return success(await res.json() as AiConfig);
      } catch (error) {
        return failure(error);
      }
    },
    ...
    listAiModels: async (): Promise<BridgeResult<AiModelsResult>> => {
      try {
        const res = await fetch(`${backendClient.baseUrl}/api/ai/models`);
        return success(await res.json() as AiModelsResult);
      } catch (error) {
        return failure(error);
      }
    },
    ...
    // Auto-update is Electron-only; browser polyfill returns no-ops.
...
function emitBackendLog(activeRun: ActiveBrowserRun, emitRunEvent: (event: RunEvent) => void, log: BackendTaskLogEntry): void {
  ...
  // When the backend 'input' log is delivered (via WS or HTTP poll), mark the prompt as handled
  // so the poll-based fallback in pollBackendTask won't emit a duplicate synthetic entry.
  if (level === 'input') {
