...
// Action type prefixes that map to real backend runners; 'control.noop' is the only exception.
const EXECUTABLE_ACTION_PREFIXES = ['browser.', 'ui.', 'http.', 'script.', 'data.', 'file.', 'excel.', 'variable.', 'control.'];
// Variables automatically injected by the backend at task start — always available at runtime.
...
/**
 * Pre-flight validator run before starting or debugging a flow. Checks
 * reachability, node configuration, flow structure (conditions/loops), and
 * variable dependencies in topological order.
 */
...
/** Returns the first error-severity issue that should block the run, or null if the flow can proceed. */
...
/**
 * Variant of `validateNodeConfiguration` that also includes flow-structural
 * and variable-dependency issues scoped to a single node, used by the
 * property panel to show inline warnings.
 */
...
/** DFS traversal starting from `startNodeId`; returns node IDs in visit order. */
...
function validateNodeAction(node: Node<RpaNodeData>, action: RpaNodeAction): RunValidationIssue[] {
  ...
  if (type === 'browser.click' || type === 'browser.wait' || type === 'browser.extract' || type === 'browser.screenshot' || type === 'ui.click' || type === 'ui.wait' || type === 'ui.extract' || type === 'ui.screenshot') {
    requireField(action.selector, '选择器');
  }
  ...
  if (type === 'browser.open' || type === 'browser.tab.open') {
...
/**
 * Walks nodes in topological order, tracking which variables have been
 * produced so far and flagging references to variables not yet defined.
 */
...
/**
 * Returns true when `name` is satisfied by the available set.
 * Dot-path access (e.g. `obj.key`) is considered available when the root
 * name (`obj`) is in the set.
 */
...
function validateFlowStructure(
  ...
  for (const nodeId of reachableNodeIds) {
    ...
    if (actionType === 'control.foreach' || actionType === 'control.loop' || actionType === 'control.for-each') {
...
/**
 * Classifies outgoing edges of a loop node into body vs. exit edges using
 * label/handle keywords. Unlabelled edges are treated as body-first when no
 * explicit body edge is found.
 */
...
/**
 * Extracts bare variable identifiers from a condition expression such as
 * `status == "done"`. Template `${var.x}` references are stripped first;
 * keywords and numeric literals are excluded.
 */
