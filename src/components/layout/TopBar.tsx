import {
  ...
  Play, Save, Square, Trash2, Variable, Workflow, XCircle,
  CheckCircle2, Bug,
} from 'lucide-react';
...
export function TopBar({
  ...
  return (
    <header className="flex h-12 shrink-0 items-center overflow-hidden border-b border-slate-200/60 bg-white/97 px-4 backdrop-blur-sm shadow-[0_1px_3px_rgba(15,23,42,0.04)]">
      ...
      {/* ── 面包屑 + 流程名编辑 ── */}
      <div className="flex min-w-0 shrink-0 items-center gap-1.5 text-[11px] text-slate-400">
      ...
      <FlowVariablesDialog
        ...
        onOpenChange={setFlowVariablesOpen}
      ...
      <VersionHistoryDialog
        ...
        onRestoreSnapshot={(version) => electron.rollbackFlowById(version).then(() => setVersionHistoryOpen(false))}
...
function StatusPill({ electron }: { electron: ElectronBridgeState }): ReactElement {
  ...
  const cfg: Record<typeof runtimeStatus, { icon: ReactElement; label: string; cls: string }> = {
    ...
    stopped: { icon: <Square className="h-3 w-3" strokeWidth={1.5} />, label: '已停止', cls: 'border-amber-200 bg-amber-50 text-amber-700' },
  };
  ...
  return (
    <DropdownMenu onOpenChange={(open) => { if (open) void onLoadFlows(); }}>
      ...
      <DropdownMenuContent align="start" className="w-72">
        ...
        <DropdownMenuSeparator />
        <div className="px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-400">最近版本</div>
        <div className="mb-1 rounded-lg bg-slate-50 px-3 py-2 text-[10px] leading-5 text-slate-500">
          {dirty ? '当前画布有未保存修改' : '当前画布已与版本快照同步'}
          {lastAutosavedAt !== null && (
            <span className="block font-mono text-slate-400">本地草稿 {formatFlowTime(lastAutosavedAt)}</span>
          )}
        </div>
        {activeFlowId !== undefined ? (
          ...
          <div className="px-3 py-2 text-[11px] text-slate-400">暂无已保存流程</div>
        )}
      </DropdownMenuContent>
    </DropdownMenu>
  );
