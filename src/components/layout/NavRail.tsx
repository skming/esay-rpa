...
export function NavRail({
  ...
  return (
    <nav
      className={cn(
        'nav-glass relative z-10 flex shrink-0 flex-col transition-[width] duration-250 ease-out',
        collapsed ? 'w-12' : 'w-44',
      )}
    >
      {/* 品牌区 */}
      <div className={cn('flex h-14 shrink-0 items-center', collapsed ? 'justify-center' : 'justify-between px-3')}>
        {collapsed ? (
          ...
          <>
            ...
            <button
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-slate-400 transition-colors duration-150 hover:bg-slate-100 hover:text-slate-600"
      ...
      {collapsed && (
        <button
          className="absolute right-0 top-13 z-20 flex h-5 w-5 -translate-y-1/2 translate-x-1/2 items-center justify-center rounded-full border border-rule bg-white text-slate-400 shadow-sm transition-all duration-150 hover:border-slate-300 hover:bg-slate-50 hover:text-accent-strong"
