...
// ── NavRail variant (collapsed / expanded) ───────────────────────────────────
    ...
    >
      ...
      {unreadCount > 0 && (
        <span className={cn(
          ...
          collapsed ? 'absolute right-1.5 top-1.5 h-3.5 min-w-[14px] px-0.5' : 'h-4 min-w-[16px] px-1',
        )}>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <span className="mt-1.5 block font-mono text-[10px] text-ink-4">{formatNotificationTime(n.at)}</span>
      </div>
    </button>
  );
}
