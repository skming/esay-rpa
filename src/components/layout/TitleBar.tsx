  ...
  return (
    <header className="drag-region flex h-9 shrink-0 items-center border-b border-slate-200 bg-white px-3 text-[11px]">
      ...
      <div className="flex flex-1 items-center justify-end gap-1.5">
        ...
        <span className="ml-1 w-12 text-right font-mono text-[11px] text-slate-400">{clock}</span>
      </div>
    </header>
  );
}
...
const STATUS_LABEL: Record<ElectronBridgeState['runtimeStatus'], string> = {
  error: '失败', ready: '', running: '运行中', stopped: '已停止', success: '已完成'
};
  ...
  return (
    <NotificationDropdown side="bottom" align="end">
      <div className="relative">
        ...
        {unreadCount > 0 && (
          <span className="pointer-events-none absolute -right-0.5 -top-0.5 flex h-3.5 min-w-[14px] items-center justify-center rounded-full bg-red-500 px-0.5 font-sans text-[8px] font-semibold leading-none text-white">
