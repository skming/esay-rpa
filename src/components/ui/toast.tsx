...
>(({ className, ...props }, ref): ReactElement => (
  <ToastPrimitive.Viewport
    className={cn(
      'fixed top-4 left-1/2 z-[120] flex w-max max-w-[calc(100vw-32px)] -translate-x-1/2 flex-col items-center gap-2 outline-none',
      className
    )}
    ref={ref}
    {...props}
...
>(({ className, ...props }, ref): ReactElement => (
  <ToastPrimitive.Close
    className={cn(
      '-mr-1 ml-1 rounded-full p-0.5 text-slate-400 opacity-60 outline-none transition-opacity hover:opacity-100',
      className
    )}
    ref={ref}
    {...props}
