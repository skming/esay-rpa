...
const DialogOverlay = forwardRef<ElementRef<typeof DialogPrimitive.Overlay>, ComponentPropsWithoutRef<typeof DialogPrimitive.Overlay>>(
  ({ className, ...props }, ref): ReactElement => (
    <DialogPrimitive.Overlay className={cn('fixed inset-0 z-90 bg-slate-950/45 backdrop-blur-[2px] data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:duration-200 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:duration-150', className)} ref={ref} {...props} />
  )
);
...
const DialogHeader = ({ className, ...props }: ComponentPropsWithoutRef<'div'>): ReactElement => (
  <div className={cn('flex shrink-0 items-start justify-between gap-4 px-6 pb-4 pt-6 pr-12', className)} {...props} />
);
