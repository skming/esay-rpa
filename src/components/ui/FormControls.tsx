...
export function TextareaField({
  ...
  return (
    <Label className="block">
      ...
      <textarea
        className={cn(
          'w-full resize-y rounded-md border border-slate-200 bg-white px-3 py-2 text-[11px] leading-relaxed text-slate-700 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-accent focus:ring-2 focus:ring-accent-soft',
          mono && 'font-mono',
        )}
        onChange={onChange}
        placeholder={placeholder}
...
/** Segmented control (§14.4) — mutually exclusive 2–3 option switch. */
  ...
  return (
    <div>
      ...
      <div className="grid h-8 grid-cols-[32px_1fr_32px] overflow-hidden rounded-md border border-slate-200 bg-white">
        <Button aria-label={`${label} 减少`} className="rounded-none border-0 border-r border-slate-200 text-slate-400 hover:bg-slate-50 hover:text-slate-700" onClick={() => onStep?.(-5)} size="icon" variant="outline">
        ...
        <Button aria-label={`${label} 增加`} className="rounded-none border-0 border-l border-slate-200 text-slate-400 hover:bg-slate-50 hover:text-slate-700" onClick={() => onStep?.(5)} size="icon" variant="outline">
