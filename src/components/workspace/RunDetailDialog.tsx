          ...
          {(run.artifacts?.length ?? 0) > 0 && (
            <div>
              <div className="mb-1.5 flex items-center justify-between gap-2 text-[11px] font-semibold text-slate-600">
                ...
                {onOpenArtifact !== undefined && firstArtifact !== undefined && (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        aria-label="打开产物所在文件夹"
                        className="rounded p-1 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700"
                        onClick={() => onOpenArtifact(firstArtifact)}
                        type="button"
                      >
                        <FolderOpen className="h-3.5 w-3.5" strokeWidth={1.5} />
                      </button>
                    </TooltipTrigger>
                    <TooltipContent side="left">打开所在文件夹</TooltipContent>
                  </Tooltip>
                )}
              </div>
