...
import { Figure, KeyRow, Panel, StatBand } from './surfaces';
...
export function StatisticsPage({ electron }: { electron: ElectronBridgeState }): ReactElement {
  ...
  return (
    <WorkspaceShell
      actions={
        <RefreshButton variant="subtle" onClick={async () => { await electron.loadQueueStats(); await electron.loadRuns({ limit: 20 }); }}>
          刷新指标
        </RefreshButton>
      }
    ...
    >
      <StatBand>
        <Figure first label="当前进度" value={progressPercent} note="本次" tone={electron.progress.percent > 0 && electron.progress.percent < 100 ? 'live' : 'ink'} />
...
function LogCell({ label, value, tone = 'ink', first }: { label: string; value: number; tone?: 'ink' | 'live'; first?: boolean }): ReactElement {
  return (
    <div className={`flex items-center gap-2 px-4 py-2.5 ${first ? '' : 'rule-v'}`}>
      <span className="text-[11px] font-medium leading-none text-ink-3">{label}</span>
      <span className={`figure text-[15px] leading-none ${tone === 'live' && value > 0 ? 'text-live' : 'text-ink'}`}>
        {value}
      </span>
    </div>
  );
