...
import { LedgerEmpty, Panel, StateTag } from './ledger';
    ...
    >
      {runs.length === 0 ? (
        ...
        <div className="max-h-120 overflow-y-auto">
          {/* Column header — Inter label row, unified with the RPA-tool register */}
          <div className="sticky top-0 z-10 grid grid-cols-[minmax(0,1fr)_104px_92px_84px_148px_44px] items-center gap-3 border-b border-rule-2 bg-surface px-5 py-2.5 text-[11px] font-medium text-ink-3">
            <span>流程</span>
            <span>状态</span>
            <span>耗时</span>
            <span>输出</span>
          ...
          {runs.map((run) => {
            ...
            return (
              <div
                key={run.taskId}
                className="grid grid-cols-[minmax(0,1fr)_104px_92px_84px_148px_44px] items-center gap-3 border-b border-rule px-5 py-3 transition-colors duration-150 last:border-b-0 hover:bg-paper-sunk"
              >
