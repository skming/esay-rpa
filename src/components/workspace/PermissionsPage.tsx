...
import { KeyRow, Panel, StatBand, Figure } from './surfaces';
...
export function PermissionsPage({ electron }: { electron: ElectronBridgeState }): ReactElement {
  return (
    <WorkspaceShell
      actions={
        <RefreshButton variant="subtle" onClick={() => electron.loadQueueStats()}>同步状态</RefreshButton>
      }
    ...
    >
      <StatBand>
        <Figure
          first
          label="桥接状态"
          value={electron.available ? '已连接' : '未连接'}
          note={electron.available ? 'ONLINE' : 'OFFLINE'}
          tone={electron.available ? 'live' : 'ink'}
        />
        <Figure label="主机" value={electron.appInfo?.hostname ?? '--'} note="HOST" />
        <Figure label="平台" value={electron.appInfo?.platform ?? '--'} note="OS" />
      </StatBand>
