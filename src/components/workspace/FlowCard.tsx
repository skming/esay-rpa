import { Archive, CirclePause, CirclePlay, Download, History, MoreHorizontal, Play, PowerOff, Square, SquarePen, TimerReset, Trash2 } from 'lucide-react';
...
export function FlowCard({
  ...
  return (
    <div
      className={cn(
        'group flex w-full flex-col gap-3 p-4',
        SURFACE,
    ...
    >
            ...
            {item.state === 'running' && (
              <>
                ...
                <DropdownMenuItem className="text-red-600 focus:text-red-600" onSelect={onStop}>
                  <Square className="mr-2 h-3.5 w-3.5" fill="currentColor" strokeWidth={1.5} />
                  停止运行
                </DropdownMenuItem>
                <DropdownMenuSeparator />
              </>
      ...
      {item.state === 'running' && (
        <div className="flex items-center gap-2">
          <div className="h-0.75 flex-1 overflow-hidden rounded-full bg-live-soft">
            <div className="h-full w-2/3 rounded-full bg-live" />
          </div>
          <Button
            className="h-6 shrink-0 gap-1 rounded-md px-2 text-[10.5px] text-red-600 hover:text-red-700"
            onClick={onStop}
            variant="subtle"
          >
            <Square className="h-2.5 w-2.5" fill="currentColor" strokeWidth={1.5} />
            停止
          </Button>
        </div>
      )}
