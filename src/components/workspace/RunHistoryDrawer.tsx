...
const RUN_STATE: Record<TaskSnapshot['status'], { state: LedgerState; label: string }> = {
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogPrimitive.Portal>
        {/* Overlay */}
        <DialogPrimitive.Overlay className="fixed inset-0 z-80 bg-slate-950/20 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:duration-200 data-[state=closed]:duration-150" />

        {/* Panel */}
        <DialogPrimitive.Content
          aria-describedby={undefined}
          className="fixed right-0 top-0 z-81 flex h-full w-175 max-w-[95vw] flex-col bg-surface shadow-2xl outline-none border-l border-rule data-[state=open]:animate-in data-[state=open]:slide-in-from-right data-[state=open]:duration-300 data-[state=closed]:animate-out data-[state=closed]:slide-out-to-right data-[state=closed]:duration-200"
        >
          {/* ── Header ─────────────────────────────────────────────── */}
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            {runs.length === 0 ? (
              ...
              <TooltipProvider delayDuration={400}>
                {/* Column header */}
                <div className="sticky top-0 z-10 grid grid-cols-[56px_92px_80px_88px_minmax(0,1fr)_44px] items-center gap-3 border-b border-rule-2 bg-paper-sunk px-5 py-2.5 text-[11px] font-medium text-ink-3">
                  <span>状态</span>
                  <span>耗时</span>
                  <span>步骤</span>
                  <span>输出</span>
                ...
                {runs.map((run) => {
                  ...
                  return (
                    <div
                      key={run.taskId}
                      className="grid grid-cols-[56px_92px_80px_88px_minmax(0,1fr)_44px] items-center gap-3 border-b border-rule px-5 py-3 transition-colors last:border-b-0 hover:bg-paper-sunk"
                    >
                      {/* 状态 */}
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="truncate font-mono text-[10px] text-ink-4">{run.taskId}</div>
                      </div>
