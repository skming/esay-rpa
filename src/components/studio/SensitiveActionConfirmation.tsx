...
type Parsed = { title: string; body: string; timeoutMs: number };
...
function parseMessage(raw: string): Parsed {
  ...
  const nl = withoutTimers.indexOf('\n');
  const title = nl >= 0 ? withoutTimers.slice(0, nl).trim() : (withoutTimers || '确认敏感操作');
  const body = nl >= 0 ? withoutTimers.slice(nl + 1).trim() : '';

  return { title, body, timeoutMs };
}
...
export function SensitiveActionConfirmation({ message, onConfirm, onStop }: SensitiveActionConfirmationProps): ReactElement | null {
  const active = message !== null;
  const { title, body, timeoutMs } = active
    ? parseMessage(message)
    : { title: '', body: '', timeoutMs: 300_000 };
  ...
  return (
    <div className="animate-in fade-in-0 slide-in-from-top-3 fixed left-1/2 top-4 z-(--z-banner) w-130 -translate-x-1/2 overflow-hidden rounded-xl bg-white shadow-[0_12px_40px_rgba(15,23,42,0.14),0_2px_8px_rgba(15,23,42,0.06)] duration-200">
      ...
      <span className="sr-only" role="alert">
        {body.length > 0 ? `需要确认敏感操作：${title}。${body}` : `需要确认敏感操作：${title}`}
      </span>
