...
// Models that are known to NOT support image/vision input
const NO_VISION_MODELS = new Set([
...
export function ChatInput({
  ...
  pending: boolean;
  /** True while the AI is emitting reasoning tokens but hasn't started the reply yet. */
  thinking?: boolean;
  history: string[];
  autoFocus?: boolean;
  model: string;
  onModelChange: (model: string) => void;
  /** External text to load into the composer (e.g. a starter prompt). */
  draft?: string | null;
  ...
  useEffect(() => {
    if (text === '') setHistIdx(-1);
  }, [text]);

  // ── Load external draft (starter prompt) into the composer ──────────────────
  useEffect(() => {
  ...
  // ── Auto-grow textarea to fit content (Codex-style) ─────────────────────────
  useLayoutEffect(() => {
    const el = textareaRef.current;
    if (!el) return;
  ...
  const visionWarning = attachments.some(a => a.type === 'image') && NO_VISION_MODELS.has(model);
  ...
  return (
    <div className="shrink-0 bg-white px-3 pb-2.5 pt-1.5">
      ...
      {/* Composer card — textarea + inline toolbar in one bordered surface */}
      <div
        className={cn(
      ...
      >
        ...
        <textarea
          ref={textareaRef}
          className="block w-full resize-none bg-transparent px-3.5 pt-3 text-[12.5px] leading-relaxed text-slate-700 outline-none placeholder:text-slate-400"
        ...
        <div className="flex items-center gap-1 px-2 pb-2 pt-1">
          ...
          {/* Thinking indicator — shown while reasoning tokens arrive before reply */}
          {pending && thinking && (
