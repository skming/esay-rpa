          ...
          <DropdownMenuContent align="end" className="w-72 p-2">
            ...
            {groupedRows.length === 0 ? (
              <div className="grid place-items-center gap-1 rounded-md border border-dashed border-slate-200 bg-slate-50 px-3 py-5 text-center">
                ...
                <div className="text-[10px] text-slate-400">{mode === 'reference' ? '可继续手动输入变量名' : '可继续输入新的输出变量名'}</div>
              </div>
            ) :
              <div className='max-h-72 overflow-y-auto'>
                {
                  groupedRows.map((group, groupIndex) => (
                    <div key={group.scope}>
                      ...
                      <div className="px-2 py-1 text-[10px] font-bold uppercase tracking-normal text-slate-400">{group.scope}变量</div>
                      {group.rows.map((row) => (
