import { CalendarClock, Clock3, Puzzle, Play, RotateCcw, TimerReset } from 'lucide-react';
...
function buildInitialDraft(schedule?: ScheduleSnapshot): ScheduleDraft {
  if (schedule !== undefined) {
    return {
      browserExecutor: schedule.task.browserExecutor ?? 'playwright',
      conflictPolicy: 'skip',
...
export function ScheduleCreateDialog({
  ...
  const handleFlowChange = (flowId: string): void => {
    setDraft((current) => {
      const selected = flows.find((f) => f.flowId === flowId);
      return { ...current, flowId, browserExecutor: selected?.defaultBrowserExecutor ?? 'playwright' };
    });
    setError(null);
  };
  ...
  const handleSubmit = (): void => {
    // 后端调度目前只认 Cron，Interval 模式在提交时换算成等价的“每 N 分钟”表达式
    ...
    // conflictPolicy/maxInstances/retryTimes/retryDelaySeconds 后端尚未接收，填了也不会保存
    const options: CreateScheduleOptions = {
      browserExecutor: draft.browserExecutor,
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent className="w-180">
        ...
        <DialogBody className="grid gap-3">
          ...
          <Label className="grid gap-1">
            <span>执行流程</span>
            <Select onValueChange={handleFlowChange} value={draft.flowId}>
              <SelectTrigger>
          ...
          <div className="flex h-8 items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-3 text-[11px] text-slate-600">
            ...
            <Switch
              ...
              checked={draft.browserExecutor === 'extension'}
              onCheckedChange={(checked) => updateDraft('browserExecutor', checked ? 'extension' : 'playwright')}
            />
          </div>
          {draft.browserExecutor === 'extension' && (
            <p className="-mt-2 text-[10px] text-amber-700">
              定时任务在后台无人值守运行；插件模式依赖一个已打开、已登录的真实浏览器窗口，若触发时窗口未打开可能导致本次执行失败，请确认执行环境后再开启。
            </p>
          )}
