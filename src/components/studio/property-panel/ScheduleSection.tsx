  ...
  return (
    <PanelSection title="调度运行">
      ...
      {latestSchedules.length === 0 ? (
        <div className="rounded-md border border-dashed border-slate-200 bg-slate-50 px-2 py-3 text-center text-[10px] leading-4 text-slate-400">暂无调度，创建后可在此启停、触发或删除。</div>
      ) : (
    ...
    <div className="rounded-md border border-slate-200 bg-white p-2 shadow-xs">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="mt-0.5 truncate font-mono text-[10px] text-slate-400">{schedule.cronExpression}</div>
        </div>
