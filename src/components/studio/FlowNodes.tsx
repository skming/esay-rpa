...
/* Status drives the most prominent signal on the canvas: a full-width top bar
   (color = state, shimmering when live) plus an explicit labeled pill. State
   transitions read at a glance from across the graph. */
const NODE_STATUS: Record<NodeStatus, {
  ...
  pending: {
    ...
    bar: 'bg-slate-200',
    pill: 'bg-slate-100 text-slate-400 ring-1 ring-slate-200/70'
  },
  skipped: {
    label: '跳过', icon: Ban,
    bar: 'bg-slate-200',
  ...
  return (
    <div className="group relative w-60">
      ...
      >
        ...
        {data.breakpoint && (
          <span
            className="absolute left-2.5 top-2.5 z-10 grid h-2.5 w-2.5 place-items-center rounded-full bg-red-500 ring-2 ring-white"
            title="断点已启用"
          >
        ...
        {data.validationSeverity !== undefined && data.validationCount !== undefined && (
          <span
            className={cn(
              'absolute right-2.5 top-2.5 z-10 inline-flex min-w-4.5 items-center justify-center rounded-full border px-1 py-0.5 text-[9px] font-bold leading-none ring-2 ring-white',
        ...
        <div className="flex items-center gap-3 px-3 pb-3 pt-3.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 truncate font-mono text-[10px] leading-none text-slate-400">
              {data.description}
            </div>
          </div>
