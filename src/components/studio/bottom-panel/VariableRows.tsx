...
export function VariableRows({ rows }: { rows: RuntimeVariableView[] }): ReactElement {
  ...
  const startEdit = (row: RuntimeVariableView): void => {
    ...
    // 敏感变量编辑框留空而非填入掩码字符串，避免用户误提交 "••••••••" 当作新值
  ...
  const renderRow = (row: RuntimeVariableView, isGlobal = false): ReactElement => {
    ...
    // 只有仍取默认值的变量允许内联编辑；本次覆写值/运行时值是只读的（改了也不会生效）
      ...
      >
        ...
        <TableCell className="w-25 pr-2">
          <div className="flex items-center gap-0.5">
            {canEdit && !isEditing && (
              <Button className="h-6 px-1.5 text-[10px] text-slate-500 hover:bg-accent-soft hover:text-accent-strong" onClick={() => startEdit(row)} title="编辑变量值" variant="ghost">
                <Pencil className="h-3 w-3" strokeWidth={1.5} />编辑
              </Button>
            )}
            {isEditing && (
              <Button className="h-6 px-1.5 text-[10px] text-slate-500 hover:bg-slate-100 hover:text-slate-700" onClick={cancelEdit} title="取消编辑 (Esc)" variant="ghost">
