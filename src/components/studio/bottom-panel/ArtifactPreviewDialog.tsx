...
// File extensions handled by @silurus/ooxml
...
// Markdown file extensions rendered as rich text
...
// Some markdown artifacts are delivered as a base64 data URL (when the backend
// could not resolve a text/* mime type). Decode those back to UTF-8 source so the
// renderer never receives the opaque base64 string.
...
export function ArtifactPreviewContent({ content, loading }: { content: ArtifactContent | null; loading?: boolean }): ReactElement {
  ...
  const { filename, contentType } = content.artifact;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-[oklch(0.175_0.018_264)]">
      ...
      {isImage ? (
        ...
        <OoxmlPreview content={content.content} ext={ooxmlExt} filename={filename} />
      ) : isMarkdown ? (
...
function OoxmlPreview({ content, ext, filename }: { content: string; ext: string; filename: string }): ReactElement {
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col bg-white">
      {/* Loading overlay */}
      {status === 'loading' && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-2.5 bg-white">
          <Loader2 className="h-5 w-5 animate-spin text-slate-300" strokeWidth={1.5} />
          <span className="text-[11px] text-slate-400">正在加载文件…</span>
        </div>
      )}

      {/* Page navigation for paged viewers (docx / pptx) */}
      ...
      {/* XlsxViewer mounts into this div and manages its own canvas + tab bar */}
      {isXlsx && (
        <div className="relative min-h-0 flex-1 overflow-hidden" ref={containerRef} />
      )}

      {/* DocxViewer / PptxViewer render into a caller-provided canvas */}
      {!isXlsx && (
...
// ── Markdown viewer (.md reports) ─────────────────────────────────────────────
// Reports are documents, so render on a light reading surface with a centered
// measure rather than the dark code-style raw view.
      ...
      <article className="mx-auto max-w-[72ch] px-8 py-7 text-[13px] leading-relaxed text-slate-700">
        <ReactMarkdown
          ...
          components={{
            ...
            blockquote: ({ children }) => (
              <blockquote className="my-3 border-l-[3px] border-accent-linebg-accent-soft py-1 pl-4 text-accent italic">{children}</blockquote>
            ),
