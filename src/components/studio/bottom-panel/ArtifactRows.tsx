...
export function ArtifactRows({ artifactContent, isMockRun, lastRunId, rows, onReadArtifact }: ArtifactRowsProps): ReactElement {
  ...
  // 只认 id 匹配当前预览目标的内容，切换预览后旧请求的迟到结果不会被采用
  const localContent = previewingId !== null && artifactContent?.artifact.artifactId === previewingId
    ? artifactContent
    : null;
  const localLoading = previewingId !== null && localContent === null;
                ...
                <TableCell className="pl-2">
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="truncate font-mono text-[10px] text-slate-500">{artifact.artifactId}</div>
                    </div>
                  </div>
                </TableCell>
                <TableCell>
