...
// Duplicate toasts within this window (ms) are silently dropped to avoid spam.
const TOAST_DEDUPE_WINDOW_MS = 2_000;

/**
 * Central hook that wires together the Electron/browser bridge, run-event
 * streaming, and all flow/task actions. Returns the full `ElectronBridgeState`
 * consumed by the canvas page.
 */
export function useElectronBridge({
  ...
  // In a browser environment, fall back to the HTTP-based polyfill when the Electron preload is absent.
  ...
  // Returns the assigned toast ID so callers can optionally dismiss it programmatically.
  const pushToast = useCallback((type: BridgeToast['type'], message: string, icon?: string): number => {
    const now = Date.now();
    let assignedId = 0;
    setToasts((current) => {
      ...
      return [
        { id: assignedId, type, message, icon },
  ...
  /** Unwraps a BridgeResult, showing a toast on error or success and returning data or null. */
  ...
  const setBottomPanelActiveTab = useBottomPanelStore((state) => state.setActiveTab);
  ...
  const onArtifactsReady = useCallback(() => {
    ...
    setBottomPanelActiveTab('artifacts');
  }, [setBottomPanelActiveTab, setBottomPanelOpen]);
  ...
  // Written by useElectronBridgeActions before startRun; read by useRunEventHandler on finish.
  ...
  }, [currentFlow?.flowId, currentFlow?.inputVariables, replaceAllInputVariables]);
  ...
  // Only apply stored overrides when they belong to the currently open flow; stale overrides are discarded.
