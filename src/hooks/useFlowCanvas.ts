...
export function useFlowCanvas() {
  ...
  // Ensure a valid node is always selected when the node list changes
  useEffect(() => {
    if (selectedNode !== undefined || flowNodes.length === 0) return;
    setSelectedNodeId(flowNodes[0]?.id ?? 'start');
