...
class Settings:
    ...
    All paths default to subdirectories of ~/.easy-rpa unless overridden via RPA_* env vars.
    """
    ...
    workspace_root: str = ""
    log_level: str = "INFO"
    log_backup_count: int = 30  # days of daily log files to retain
    log_module_levels: dict[str, str] = field(default_factory=dict)


def load_settings() -> Settings:
    """Reads all RPA_* environment variables and returns a validated Settings instance."""
...
def _read_module_levels(name: str) -> dict[str, str]:
    """Parse RPA_LOG_LEVELS=module:LEVEL,module:LEVEL into a dict.

    Example: ``app.services.scheduler_service:WARNING,litellm:ERROR``
    """
