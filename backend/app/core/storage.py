"""~/.easy-rpa 下运行时文件的统一目录布局与命名/清理规则，供各执行器共用。

  <app_data>/
    workspace/  用户可见工作目录（脚本 cwd），runs/<flow_key>/<task_id>/<run_timestamp>.<ext>
    runtime/    服务可变状态：browser/profile、browser/cookies.json、scrapling
    ai/         config.json、chats/
    cache/      scripts/inline_<hash>.py（按时间/数量淘汰）
    logs/
"""
...
DEFAULT_OUTPUT_RETENTION = 10  # 每个流程默认只保留最近 10 次运行的产物目录
...
def resolve_playwright_browsers_dir() -> Path:
    """浏览器内核目录，必须与 electron/backendSupervisor.cjs 的 playwrightBrowsersDir 同址。"""
    return resolve_runtime_root() / "playwright-browsers"
    ...
    *,
    ...
    for run_dir in ranked[keep:]:
        try:
            ...
            pass  # 目录被占用等失败情况故意忽略，不影响本次任务，下次清理再重试
    return removed
...
def prune_temp_scripts(
    *,
    ...
    """按年龄和数量上限清理内联脚本缓存；这些文件按需重新生成，删除总是安全的。"""
    directory = (cache_dir or resolve_cache_dir()) / "scripts"
