...
# All node types that this runner handles; used for fast membership checks.
...
class VariableActionResult:
    """Return value from VariableActionRunner.run(), carries output values and the log level to emit."""
...
class VariableActionRunner:
    """Executes variable/message nodes (set, get, input, log, notify, clipboard) synchronously."""
...
def apply_variable_result_variables(node: FlowNode, result: VariableActionResult, variables: RuntimeVariableStore) -> list[str]:
    """Writes result values into the variable store and returns the list of variable names that were updated."""
