...
from sqlalchemy import BigInteger, DateTime, Integer, String, Text, delete, inspect, select, text
...
class TaskStore(Protocol):
    """Storage interface for task snapshots, logs, and variables.

    Implemented by `InMemoryTaskStore` (for tests/dev) and `SqlAlchemyTaskStore`
...
class InMemoryTaskStore:
    """Dict-backed task store used in development and tests; data is lost on process restart."""
...
class SqlAlchemyTaskStore:
    """Async SQLAlchemy task store backed by SQLite or PostgreSQL."""
...
def _ensure_task_columns(connection) -> None:
    """Add columns introduced in later migrations without dropping existing data."""
