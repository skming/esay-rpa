"""编排层硬护栏：按优先级检查当前调用参数与授权，不依赖失败历史。

阶段准入与失败预算由 ai_phases 管理。guard_contract_lines 从策略表生成提示词契约，
拦截结果通过稳定的 guard_id 标识对应规则。
"""
...
def _check_schedule_authorization(
    ...
    """创建定时任务必须有用户授权，因为 enabled 默认 true，会触发无人值守执行。

    授权依据整段会话，避免后续“继续”覆盖最初的定时要求；停用任务不受此限制。"""
