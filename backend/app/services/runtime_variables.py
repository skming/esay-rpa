...
# Matches ${var.name} interpolation placeholders in string config values.
_VARIABLE_PATTERN = re.compile(r"\$\{var\.([A-Za-z_][A-Za-z0-9_.-]{0,119})\}")
# Guard against circular/deeply-nested template references.
_MAX_TEMPLATE_RESOLUTION_DEPTH = 5
...
class RuntimeVariableStore:
    ...
    def raw_values(self) -> dict[str, object]:
        """Return variables as native Python objects (no JSON stringification)."""
...
def apply_variable_node(node: dict[str, object], variables: RuntimeVariableStore) -> bool:
    node_type = node.get("type")
    if node_type not in {"variable.step", "variable.set", "variable.assign"}:
        return False

    name = read_variable_name(node)
    raw_value = node.get("value", node.get("inputValue", node.get("defaultValue", "")))
    value = resolve_template_value(raw_value, variables)
...
def apply_fetch_result_variables(node: dict[str, object], result: ScrapeResult, variables: RuntimeVariableStore) -> list[str]:
    ...
    for key in ("outputVariable", "responseVariable", "saveAs", "resultVariable"):
        ...
        if isinstance(raw_name, str) and raw_name.strip():
            variables.set(raw_name, list(result.values), scope="局部")
    ...
    if isinstance(first_value_name, str) and first_value_name.strip():
        variables.set(first_value_name, result.values[0] if result.values else "", scope="局部")
        saved_names.append(normalize_variable_name(first_value_name))

    _append_result_variable(node, variables, values=list(result.values), count=result.count, saved_names=saved_names)
    return saved_names


def normalize_variable_name(value: str) -> str:
    """Validates and returns the trimmed variable name; raises ValueError for empty, too-long, or invalid names."""
    name = value.strip()
...
def _read_nested_value(name: str, values: dict[str, object]) -> object:
    # Supports dot-path access: "result.0.name" resolves into nested dict/list.
    parts = name.split(".")
