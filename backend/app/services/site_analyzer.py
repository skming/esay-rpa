...
class SiteAnalyzer:
    def __init__(self, fetcher: PageFetcher | HtmlFetcher | None = None) -> None:
        ...
        # 保留 analyze() 期间的 Scrapling page 对象，供 _scrapling_extra_candidates 复用
    ...
    def _build_candidates(self, document, *, limit: int) -> list[_RawCandidate]:
        ...
        # 合并 Scrapling generate_css_selector 生成的语义化最短唯一路径作为补充候选
    ...
    def _detect_css_in_js(self, document) -> bool:
        ...
        return hashed_count / len(classes) >= 0.25  # 经验阈值：四分之一以上类名疑似哈希即判定为 CSS-in-JS 站点
    ...
    def _ambiguity_penalty(self, match_count: int, tag_count: int) -> int:
        # 命中越多，selector 越不唯一，扣分越重；命中数接近同类标签总数时视为"选中了整类元素"，扣分最重
        if match_count <= 1:
            return 0
        if match_count <= 3:
            return 8
