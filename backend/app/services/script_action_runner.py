...
_MAX_STDIO_BYTES = 512_000
_MAX_ENV_BYTES = 64_000
_MAX_ENV_VALUE_BYTES = 8_000

# Preamble injected at the top of every inline Python script so that _vars is
# always sourced from the full-fidelity file snapshot, not the env-var copy
# (which is truncated for large datasets).  The sentinel prevents double injection.
...
# Old boilerplate lines generated by the AI / UI that must be stripped so they
# don't silently overwrite the injected _vars with potentially truncated data.
_PY_STALE_LOADERS = [
...
class ScriptActionRunner:
    ...
    def _resolve_script_path(self, node: FlowNode, variables: RuntimeVariableStore) -> Path:
        ...
        if raw_path:
            ...
            # 文件不存在时：若有 code 字段则自动生成，方便用户后续编辑
        ...
        # Inline scripts are ephemeral cache, not user output: keep them in the
        # dedicated cache dir (out of the user workspace) and prune old entries.
...
def _inject_py_preamble(code: str) -> str:
    """Strip stale _vars loader boilerplate and prepend the file-aware preamble."""
    if _PY_PREAMBLE_SENTINEL in code:
        return code  # already injected (e.g. path-mode script written manually)
...
def _build_variables_payload(variables: RuntimeVariableStore) -> str:
    # Use raw Python objects so lists/dicts aren't double-encoded into JSON strings.
...
def _build_script_env(variables: RuntimeVariableStore, *, restrict_path: bool) -> dict[str, str]:
    ...
    # Also expose a compact copy via env var for shell scripts and legacy code
    # that reads RPA_VARIABLES_JSON directly.
...
# Matches the same ${var.NAME} pattern used by RuntimeVariableStore.resolve_text.
...
def _resolve_shell_safe(template: str, variables: RuntimeVariableStore) -> str:
    """Substitute ${var.X} placeholders with shell-quoted values to prevent injection.

    Unlike resolve_text(), this quotes each substituted value with shlex.quote()
    before inserting it into the shell command string, so variable content
    cannot inject additional shell metacharacters.
    """
    ...
    def _quote(m: re.Match) -> str:
        ...
        val = raw.get(name)
        if val is None:
            # Keep unresolved placeholder as-is (mimics resolve_text behaviour).
            return m.group(0)
