...
_MAX_INPUT_CHARS = 512_000  # 单次输入上限，防止超大变量值拖垮解析/正则/JSON 处理
_MAX_REGEX_LENGTH = 500  # 限制正则表达式长度，降低灾难性回溯（ReDoS）风险
_MAX_LIST_ITEMS = 10_000  # 列表处理条数上限，避免无界数据撑爆内存/耗时
...
def _run_convert(node: FlowNode, variables: RuntimeVariableStore) -> DataActionResult:
    ...
    if operation == "to_int":
        try:
            ...
            raise ValueError("无法将输入转换为整数") from exc
    elif operation == "to_float":
        try:
            value = float(text.strip())
        except (ValueError, OverflowError) as exc:
            raise ValueError("无法将输入转换为浮点数") from exc
...
def _run_aes(text: str, operation: str, node: FlowNode, variables: RuntimeVariableStore) -> str:
    ...
    # 注意：节点未配置 pattern（密钥来源）时，所有流程共用同一个硬编码默认密钥，
    # 不提供真实机密性——仅适合内部混淆场景，敏感数据必须显式配置 pattern。
