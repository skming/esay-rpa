...
def is_condition_node(node: FlowNode, outgoing_edges: list[FlowEdge] | None = None) -> bool:
    """Returns True when the node should act as a branch point (has an evaluable condition expression)."""
...
def evaluate_condition(node: FlowNode, variables: RuntimeVariableStore) -> bool:
    """Evaluates the node's condition expression against the current variable store; raises ValueError on invalid input."""
...
def evaluate_condition_detail(node: FlowNode, variables: RuntimeVariableStore) -> ConditionEvaluation:
    """Evaluates a condition and returns debug detail for trustworthy runtime logs."""
...
def select_branch_edges(outgoing_edges: list[FlowEdge], condition_result: bool) -> list[FlowEdge]:
    """Picks outgoing edges matching the true/false branch label; falls back to positional order for unlabelled edges."""
