...
# Matches "flow_{UUID}" keys that map to a real saved flow.
# "flow_local-*" keys (unsaved drafts) are intentionally excluded.
_REAL_FLOW_RE = re.compile(r"^flow_(?!local-)([A-Za-z0-9_\-]+)$")

# Retention for ephemeral non-flow draft sessions. Flow-attached chats are real
# work and never auto-pruned.
...
class AiChatStore:
    ...
    def save(self, session_key: str, messages: list[dict[str, Any]]) -> None:
        ...
        # Bound the draft-session dir (best-effort) when this was a draft save.
    ...
    def _prune_draft_sessions(
        ...
        *,
        ...
        """Drop stale draft sessions in ai/chats/: those older than max_age_days,
        then cap the dir at `keep` newest. The just-saved file is never removed.
        ...
        for p in files:
            if keep_path is not None and p.resolve() == keep_path:
                survivors.append(p)  # never drop the file we just wrote
                continue
            try:
            ...
            if mtime < cutoff and _unlink(p):
                ...
                survivors.append(p)
        # Cap total (current file included) at `keep`; it's newest so it stays.
    ...
    def list_sessions(self) -> list[dict[str, Any]]:
        ...
        # Non-flow sessions: ai/chats/*.jsonl
